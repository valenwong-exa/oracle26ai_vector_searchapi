...
logger = logging.getLogger("oracle_vector_api_mini")
...
app = FastAPI(
    title="Oracle Vector Search API (MiniLM)",
    version="1.0.0",
    description="Upload documents, split/embed/store into Oracle, and run vector search with rerank (all-MiniLM-L6-v2).",
)
...
def upload_document(
    ...
    use_deepseek_summary: bool = Form(False),
    device: Literal["auto", "cuda", "cpu"] = Form("cpu"),
...
def vector_search(request: SearchRequest) -> list[SearchResultItem]:
    ...
    try:
        ...
        if request.reranker_score > 0:
            ...
            rerank_score_list = [1.0] * len(candidates)
            logger.info(
                "[%s] Rerank skipped (reranker_score=%s): using default score 1.0 for %s candidates",
                request_id,
                request.reranker_score,
                len(candidates),
            )
...
if __name__ == "__main__":
    uvicorn.run("oracle_vector_api_mini:app", host=DEFAULT_API_HOST, port=DEFAULT_API_PORT, reload=False)
